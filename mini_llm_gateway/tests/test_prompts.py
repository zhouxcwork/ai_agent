...
def test_create_and_version_conflict(client):
    ...
    assert conflict.json()["error"]["code"] == "prompt_version_conflict"
...
def test_render_success_and_missing_variable(client):
    ...
    assert missing.json()["error"]["code"] == "missing_prompt_variable"
...
def test_render_sandbox_blocks_ssti(client):
    ...
    assert blocked.json()["error"]["code"] == "prompt_render_failed"
